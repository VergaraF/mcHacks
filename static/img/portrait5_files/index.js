    ...
    $.extend($.fn.jGrowl.prototype, {
        ...
        create: function (message, o) {
            ...
            if(this.last_notification) {
                var last_message = this.last_notification.m,
                last_data = $(this.last_notification.o).data("jGrowl");
                if(last_data && last_data.preserve && last_message == message) {
                    last_data.created = new Date();
                    return;
                }
            }
            if(!o.queue) this.close();
        ...
        render: function (notification) {
            ...
            if ($('div.jGrowl-notification:parent', self.element).size() > 1 && $('div.jGrowl-closer', self.element).size() == 0 && this.defaults.closer != false) {
                $(o.closerTemplate).addClass('jGrowl-closer ui-state-highlight ui-corner-all').addClass(this.defaults.theme).appendTo(self.element).animate(this.defaults.animateOpen, this.defaults.speed, this.defaults.easing).bind("click.jGrowl", function () {
